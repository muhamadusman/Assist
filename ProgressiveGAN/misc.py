...
def save_image_grid_five_channels(images, filename, drange=[0,1], grid_size=None):
    image = create_image_grid(images, grid_size)

    thischannel = image[0,:,:]
    convert_to_pil_image(thischannel, drange).save(filename + '_T1CE.png')

    thischannel = image[1,:,:]
    convert_to_pil_image(thischannel, drange).save(filename + '_T1.png')

    thischannel = image[2,:,:]
    convert_to_pil_image(thischannel, drange).save(filename + '_FLAIR.png')

    thischannel = image[3,:,:]
    convert_to_pil_image(thischannel, drange).save(filename + '_T2.png')

    thischannel = image[4,:,:]
    convert_to_pil_image(thischannel, drange).save(filename + '_Seg.png')
